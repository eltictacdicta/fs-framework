...
class albaran_cliente extends \fs_model
{
    ...
    {
        $order = $this->safe_order_by($order, ['fecha DESC', 'fecha ASC', 'codigo DESC', 'codigo ASC']);
    ...
    {
        $order = $this->safe_order_by($order, ['fecha ASC', 'fecha DESC', 'codigo ASC', 'codigo DESC']);
    ...
    /**
     * Valida el parámetro $order contra un allowlist de columnas y direcciones.
     * Evita SQL injection cuando el valor proviene de un parámetro de usuario.
     *
     * @param string $order
     * @param string[] $allowed
     * @return string
     */
    private function safe_order_by($order, array $allowed)
    {
        if (!is_string($order) || !in_array($order, $allowed, true)) {
            return $allowed[0];
        }
        return $order;
    }
