 ...
 */
namespace FSFramework\Plugin\BusinessData\Model;
...
class Almacen extends \fs_model
{
     ...
     * @param string $cod
     * @return \FSFramework\Plugin\BusinessData\Model\Almacen|boolean
     */
    ...
    {
        if ($this->is_default()) {
            $this->new_error_msg('No puedes eliminar el almacén predeterminado.');
            return FALSE;
        }
     ...
     * @return \FSFramework\Plugin\BusinessData\Model\Almacen
     */
    public function all()
    {
        /// leemos la lista de la caché
        $almalist = $this->cache->get_array('m_almacen_all');
        if (empty($almalist)) {
            ...
            if ($data) {
                foreach ($data as $a) {
                    $almalist[] = new Almacen($a);
                }
            }

            /// guardamos la lista en caché
            $this->cache->set('m_almacen_all', $almalist);
        }
