...
Source of truth spec for the `clientes` domain inside the `clientes_core`
plugin. This spec covers the third-party identity model — the `cliente`
entity, its persistence to the `clientes` DB table, and any lifecycle
behaviour that mutates the table outside of regular CRUD (for example,
seeding, migrations, or activation-time hooks).

This spec is the canonical place where delta specs under
`openspec/changes/{name}/specs/clientes/spec.md` are merged once a
change is archived. Until then, each active change contributes its
`ADDED Requirements`, `MODIFIED Requirements` and `REMOVED Requirements`
here as a delta; the spec is read as "this canonical spec + all open
deltas".

## Domain context

The `clientes_core` plugin owns the third-party identity model in
FSFramework. The production class is `cliente` (declared in
`plugins/clientes_core/model/core/cliente.php` under the namespace
`FSFramework\model`, not the legacy global `\cliente` alias) and
persists to the `clientes` DB table whose schema is defined in
`model/table/clientes.xml`. Standard CRUD (`test()`, `save()`,
`get_new_codigo()`) is consumed as-is by every consumer of the model;
this spec is concerned only with lifecycle behaviour outside regular
CRUD — for example, the one-shot seeder that runs at plugin activation.

The framework's plugin activation pipeline invokes a static
`Init::upgrade()` method on the plugin's `Init` class via
`fs_plugin_manager::runPluginUpgrade()` (`base/fs_plugin_manager.php:643-655`).
`clientes_core` is the first adopter of that hook; subsequent plugins
that need activation-time behaviour copy the seeder shape (flag-gated,
idempotent, never throws) established by the
`default-client-on-activation` change.

The seeder is intentionally a silent bootstrap: it never flashes UI,
never logs success, and never blocks activation if the DB insert itself
fails. The trade-off is that a swallowed `\Throwable` is invisible to
the operator — operators debug seed failures via the `clientes` table
itself (the absence of the `'Cliente por defecto'` row) and via the
...
minimum acceptance contract for the activation-time seeder.
...
<!-- Source of truth. Last updated: 2026-07-26. Merged from changes/clientes-descuentos-grupo/specs/clientes/spec.md. -->
