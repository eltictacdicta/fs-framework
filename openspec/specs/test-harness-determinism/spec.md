# Spec: test-harness-determinism (canonical)
...
|----|-------------|----------|
| THD-01 | The test bootstrap MUST define every required framework constant unconditionally and MUST NOT load the application-local `config.php`; Composer and plugin composer bootstraps MUST still be registered | MUST |
...
### Requirement: THD-02 — Test FS_SECRET_KEY satisfies SecretManager validation
...
### Requirement: THD-03 — Symfony PHPUnit bridge is installed, autoloaded, and enforcing
...
### Requirement: THD-04 — Plugins suite uses bounded deterministic discovery
...
The change MUST be verified by: (1) a full suite run before the change capturing test counts and failures, (2) a full suite run after the change, (3) a repeat run confirming identical results, and (4) a test-count comparison demonstrating that the scoped Plugins glob does not silently drop plugin tests. The original baseline forecast (42 plugin test files, of which 6 `system_updater_back` files excluded) is superseded at verification time by the actual on-disk distribution; the authoritative baseline is `baseline.md` in the archived change, and any unexplained delta blocks approval.
