...
| TCP-07 | An `htmx:after:swap` re-init shim (htmx 4 colon-style event) MUST restore `initInlineEdit`/`initSortable` bindings for swapped regions. | MUST |
...
### Requirement: TCP-01 — Lazy-load family rows via hx attributes

Lazy-load of family rows MUST use hx attributes (`hx-get` to the existing fragment action, `hx-target` to the row body, `hx-swap="innerHTML"`); the replaced `$.ajax` fetch path (`loadArticulos`/`toggleFamilia` loading) MUST be removed from `catalogo-main.js`.
...
### Requirement: TCP-02 — View-mode and sort refetch via hx-get

View-mode refetch and sort refetch MUST be driven by `hx-get` requests carrying the same action and parameters as today's refetch.
...
### Requirement: TCP-03 — Load-more appends with beforeend

Load-more MUST append rows via `hx-swap="beforeend"` onto the load-more target rendered by the fragment template.
...
### Requirement: TCP-04 — Full-page filters push URL

Full-page filters (tarifa, familia, text) MUST use `hx-get` with URL push so URL and content update together.
...
### Requirement: TCP-05 — Response parity with $.ajax consumers

Responses for hx consumers MUST be identical to those for current `$.ajax` fragment consumers: same endpoints (`htmx_articulos` / `htmx_articulos_agrupados`), same query params, same fragment templates.
...
### Requirement: TCP-06 — jQuery flows preserved

JSON POST toggles, inline edit, jQuery UI sortable, and file exports MUST keep working through the existing jQuery code.
...
### Requirement: TCP-07 — afterSwap re-init shim with htmx 4 event names

An `htmx:after:swap` re-init shim (htmx 4 colon-style event) MUST restore `initInlineEdit`/`initSortable` bindings for swapped regions.

#### Scenario: afterSwap shim re-binds handlers

- **GIVEN** an innerHTML swap completed in a migrated region
- **WHEN** htmx fires `htmx:after:swap`
- **THEN** the shim re-runs `initInlineEdit`/`initSortable` for the swapped region
- **AND** both work on the new rows

### Requirement: TCP-08 — Dead plugin helper removed, core helper adopted

The plugin's dead private `is_htmx_request()` MUST be removed; HX detection MUST delegate to `fs_controller::isHtmxRequest()`.
...
### Requirement: TCP-09 — Plugin regression suite green

Plugin regression tests in `plugins/tarifario/tests/` MUST cover the pilot flows and pass under `ddev exec php vendor/bin/phpunit`.
...
### Requirement: TCP-10 — Well-formed error rows inside table context

Error responses swapped inside `<tr>` context MUST be well-formed error-row fragments, not unbalanced HTML.
