...
Change: `retire-tarif-familia-ext` — NEW capability. This spec becomes the source of truth at archive time (no existing spec to modify).
Design refs: D-1…D-3, AD-1…AD-7 of `openspec/changes/retire-tarif-familia-ext/design.md`.
...
Every familia-creating flow — the `tarif_articulos.php` JSON import, the `tarif_articulos.php` batch import, the `tarif_catalogo_view.php` batch, and the `ExcelHierarchyService` Excel wizard — MUST persist exactly two rows when creating or updating a familia: the base catalog row in `familias` (base `familia` model) and the per-tariff hierarchy row in `tarif_tarifa_familia` (its model `save()`). Hierarchy values (`madre`, `capitulo`) MUST land on the per-tariff row, never on the base familia object. `nivel` MUST be computed by the per-tariff model, not copied from the payload.
