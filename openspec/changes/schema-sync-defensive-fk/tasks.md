...
- (h) ZIP install/update coverage (user requirement, VERIFIED — see verify): ALL schema-sync entry points converge on the two fixed builders, so ZIP flows are covered WITHOUT extra code: (1) ZIP update → fs_plugin_manager::install($was_update=true) → applySchemaUpdatesOrRollback (:595/:684) → applyPluginSchemaUpdates; (2) ZIP new install → activation → enableWithoutDependencyResolution (:508) → applyPluginSchemaUpdates; (3) fs_schema::createFromXml (:111) / generateSql (:867) → collectConstraints (:182) → addForeignKeyConstraint (fixed); (4) any model instantiation → check_table → SchemaComparator::generateTable (fixed). A plugin shipping raw SQL that bypasses the framework machinery is plugin-owned and out of framework control.
